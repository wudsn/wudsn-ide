...
/**
 * <p>
 * Generates PAL VIC colors from first principles. The VIC colors
 * are constructed on a YUV color wheel at certain luminance levels
 * that depend on precise chip revision, and at certain fixed angles
 * in the UV plane, which are fixed for all chips. Additionally,
 * several effects observed on true PAL systems are emulated.
 * 
 * <ol>
 * <li>
 * The construction of the S-video signal reduces bandwidth of both
 * luminance and chrominance. This is achieved by filtering these components
 * in the horizontal direction. The luminance component is modified in order
 * to allow dot-creep or black bleed like effects by shifting the filter kernel
 * at bright colors towards right.
 * 
 * <li>
 * PAL encoding which the VIC implements only partially and which
 * causes color blending effects: the U and V components of succeeding lines
 * become in effect averaged by the monitor's PAL decoder, because in PAL
 * the color is encoded as difference to the previous line's color, but
 * VIC keeps no memory of the color angles it generated for the preceding line.
 * 
 * <li>
 * The imperfect generation of the color angles for the odd lines.
 * The color wheel should be inverted between each row to allow difference
 * encoding, but in practice the odd rows are not rotated perfectly.
 * 
 * <li>
 ...
 * 
 * <p>
 * To implement these effects, U and V are simply averaged within each
 * 3x2 pixel block, and Y is filtered according to a gaussian at the top
 * row's 3x1 pixel block.
 * 
 * <p>
 * To do this, helper tables are constructed which describe the filtered
 * colors and luminances in the 3x1 block, and another helper table that can
 * quickly look up averages between succeeding rows to achieve the full 3x2
 * filter. The YUV colors are converted to RGB colors on the second conversion.
 * 
 * <p>
 * To keep table sizes small, the pal emulation works on a 128 color palette
 * per line. Any 3 pixel block is reduced to a single color on a 128-color
 * palette, and two such colors (from current line and preceding line) are
 * put side by side and used as index to look up the final color.
 *
 * <p>
 * Since odd and even rows have slightly different colors, the colors 0..127
 * are assigned for even rows and colors 128..255 for odd rows. They are treated
 * independently at the color generation and quantization procedure. The
...
public class Palette {
	 ...
	 * what the value really is. 2.8 makes bright colors painfully bright.
	 * Many emulators have chosen 2.5. I will follow suit to not appear too different.
	 */
	private static final float NOMINAL_PAL_GAMMA = 2.5f;

	/**
	 * This tunable is used to trade between the reproduced accuracy of rarely used colors
	 * to the accuracy of common blends. 256 colors per line are nowhere near enough, so
	 * this kind of tunable is surprisingly necessary.
	 */
	...
	protected static class PaletteEntry {
		...
		protected PaletteEntry(final float luminance, final float angle,
				final int direction, final String name) {
	...
	private byte[] calculatePaletteInternal(final PaletteEntry[] colors, YUVEntry[] yuv, float saturation, float phaseShift, float tint) {
		...
		/* calculate palette rgb->index map we can use to map colors in reverse direction. */
...
//		YUVEntry[] oddPalette = new YUVEntry[256];
		...
		// Speedup by JAC!
		evenFiltered = new byte[65536]; // ]calculatePaletteInternal(colors, evenPalette, saturation, phaseShift, tint);
		oddFiltered =  new byte[65536]; // calculatePaletteInternal(colors, oddPalette, saturation, -phaseShift, tint);
...
//			break;
		case MOS6567R8:
			lum = new float[] {
					590, 1825, 950, 1380, 1030, 1210, 860, 1560,
					1030, 860, 1210, 950, 1160, 1560, 1160, 1380 };
			break;
		case MOS6569R1:
			lum = new float[] {
					630, 1850, 900, 1560, 1260, 1260, 900, 1560,
					1260, 900, 1260, 900, 1260, 1560, 1260, 1560 };
			break;
		case MOS6569R3:
			lum = new float[] {
					700, 1850, 1090, 1480, 1180, 1340, 1020, 1620,
					1180, 1020, 1340, 1090, 1300, 1620, 1300, 1480 };
			break;
		case MOS6569R4:
			lum = new float[] {
					500, 1875, 840, 1300, 920, 1100, 760, 1500,
					920, 760, 1100, 840, 1050, 1500, 1050, 1300 };
			break;
		case MOS6569R5:
			lum = new float[] {
					540, 1850, 900, 1340, 980, 1150, 810, 1520,
					980, 810, 1150, 900, 1110, 1520, 1110, 1340 };
			break;
		default:
		...
		return new PaletteEntry[] {
				new PaletteEntry(lum[0],  ANGLE_ORN, -0, "Black"      ),
				new PaletteEntry(lum[1],  ANGLE_BRN,  0, "White"      ),
				new PaletteEntry(lum[2],  ANGLE_RED,  1, "Red"        ),
				new PaletteEntry(lum[3],  ANGLE_RED, -1, "Cyan"       ),
				new PaletteEntry(lum[4],  ANGLE_GRN, -1, "Purple"     ),
				new PaletteEntry(lum[5],  ANGLE_GRN,  1, "Green"      ),
				new PaletteEntry(lum[6],  ANGLE_BLU,  1, "Blue"       ),
				new PaletteEntry(lum[7],  ANGLE_BLU, -1, "Yellow"     ),
				new PaletteEntry(lum[8],  ANGLE_ORN, -1, "Orange"     ),
				new PaletteEntry(lum[9],  ANGLE_BRN,  1, "Brown"      ),
				new PaletteEntry(lum[10], ANGLE_RED,  1, "Light Red"  ),
