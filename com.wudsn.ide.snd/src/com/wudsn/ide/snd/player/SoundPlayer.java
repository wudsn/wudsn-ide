...
public abstract class SoundPlayer {
	...
	private final static class SoundGeneratorRunnable implements Runnable {
		...
		/**
		 * Create a new runnable for the thread in which the sound player generator is
		 * executed.
		 * 
		 * @param player    The sound player,not <code>null</code>.
		 * @param generator The sound generator, not <code>null</code>.
		 * @param listener  The sound player listener or <code>null</code>.
		 */
		...
		/**
		 * Implementation of {@link Runnable} which generated the actual sound, passes
		 * it to the source data line and notifies the listener about the updates.
		 */
		@Override
		public void run() {
			synchronized (player) {
					...
					synchronized (player) {
						...
						if (listener != null) {
							...
							if (player.getPosition()
									- player.listenerUpdatedPosition >= SoundPlayerListener.POSITION_UPDATE_INCREMENT) {
	 ...
	 * implementation.
	 * 
	 * @param loaded <code>true</code> if the song player has loaded a module,
	 ...
	 * 
	 * @param fileType     The file type to be returned.
	 * @param musicAddress The music address to be used. Either an address between
	 *                     $0000 and $ffff or <code>-1</code>. The value is only
	 *                     applied, if the file type supports changing the address
	 *                     at all.
	 * 
	 * @return The content, a non-empty binary executable or <code>null</code> if
	 *         the file type is not supported.
	 * 
	 * @throws Exception If conversion to binary format is not possible.
	 */
	...
	/**
	 * This method is called by subclasses in the implementation of load(), before
	 * the actual loading starts.
	 */
	 ...
	 * @param module      The byte array in which the module shall be loaded. Only
	 *                    te byte which fit into the array are loaded.
	 ...
	 * start. Calls
	 * {@link #playInNewThread(int, SoundGenerator, SoundPlayerListener)} to start
	 * playing at the end. Implementations must be synchronized.
	 * 
	 * @param song     The song number, a non-negative integer starting at zero for
	 *                 the first song in the tune.
	 * @param listener The listener to be notified about status changes or
	 *                 <code>null</code>.
	 * @throws CoreException In case the song cannot be played.
	 */
	 ...
	 * 
	 * @param song      The song number, a non-
	 * @param generator The sound player generator, not <code>null</code>.
	 * @param listener  The sound player listener or <code>null</code>.
	 */
	 ...
	 * 
	 * @return The position in the current song as number of milliseconds since the
	 ...
	 * 
	 * @param position The position in the current song as number of milliseconds
	 *                 since the start.
	 */
