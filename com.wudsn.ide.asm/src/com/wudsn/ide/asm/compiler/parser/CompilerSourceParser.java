...
public abstract class CompilerSourceParser {
    ...
    /**
     * Enhances the file path of an include, for example adds a default
     * extension for source includes.
     * 
     * @param type
	...
	for (int lineNumber = 0; lineNumber < lines; lineNumber++) {
	    ...
	    if (compilerSourceParserLineCallback != null
		    && compilerSourceParserLineCallback.getSourceFilePath().equals(
			    compilerSourceFile.getDocumentFile().getPath())
	...
	createChild(positionStartOffset, CompilerSourceParserTreeObjectType.EQUATE_DEFINITION, symbol, symbol + " = "
		+ operand, comment);
	...
	case InstructionType.BEGIN_LOCAL_SECTION_DIRECTIVE:
	    ...
	    createChild(positionStartOffset, CompilerSourceParserTreeObjectType.LOCAL_SECTION,
		    symbolOrOperandFirstWord, symbolOrOperand, comment);
	    beginSection(startOffset, true);
