...
public final class Atari8BitUtility {
	...
	/**
	 * Mapping of the 4 bit pixel values to the corresponding color register. Make
	 * sure not to modify contents of this array.
	 */
	 ...
	 * 
	 * @param bytes  The byte array, not empty and not <code>null</code>.
	 * @param offset The offset within the byte array, a non-negative integer.
	 ...
	 * 
	 * @param bytes  The byte array, may be empty, not <code>null</code>.
	 * @param offset The offset within the byte array, a non-negative integer.
	 ...
	 * 
	 * @param data         The byte array with the packed data, may be empty, not
	 *                     <code>null</code>.
	 * @param dataOffset   The offset within data to start unpacking, a non-negative
	 *                     integer.
	 * @param dataLength   The length of the data to be unpacked, a positive
	 *                     integer.
	 * @param cprtype      The CPR packing type, either 0,1 or 2.
	 * @param unpackedData The byte array for the unpack data, must be 7680 bytes
	 *                     long.
	...
	public static boolean unpackKoala(byte[] data, int dataOffset, int dataLength, int cprtype, byte[] unpackedData) {
		...
		if (dataLength < 1) {
			throw new IllegalArgumentException(
					"Parameter 'dataLength' must be positive. Specified value is " + dataLength + ".");
		}
		int i;
		int d;
		switch (cprtype) {
		...
		for (;;) {
			...
			do {
				...
				if (cprtype == 2) {
					...
					if (i >= 7680) {
						/*
						 * if in line 192, back to odd lines in the same column; if in line 193, go to
						 * even lines in the next column
						 */
	 ...
	 * 
	 * @param data               The byte array with the packed data, may be empty,
	 *                           not <code>null</code>.
	 * @param dataOffset         The offset within data to start unpacking, a
	 *                           non-negative integer.
	 * @param dataLength         The length of the data to be unpacked, a positive
	 *                           integer.
	 * @param step               The number of "columns", a non-negative integer.
	 * @param count              The number of "lines", a non-negative integer.
	 * @param unpackedData       The byte array for the unpack data, must be 7680
	 *                           bytes long.
	 * @param unpackedDataOffset The offset within unpackedData to start unpacking,
	 ...
	 * 
	 * @param bytes The byte array, may be empty, not <code>null</code>.
	 * @return <code>true</code> if the byte array represents a valid Atari charset,
	 *         <code>false</code> otherwise.
	 * 
	 * @since 1.6.0
	 */
