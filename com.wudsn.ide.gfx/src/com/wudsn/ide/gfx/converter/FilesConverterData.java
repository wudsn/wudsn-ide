...
public final class FilesConverterData extends ConverterCommonData {
	...
	/**
	 * Gets the converter for a converter id specified in the parameters. Instances
	 * of {@link Converter} are stateless singletons within the plugin.
	 * 
	...
	public void setSourceFileBytes(int sourceFileId, byte[] bytes) {
		if (sourceFileId < 0) {
			throw new IllegalArgumentException(
					"Parameter 'sourceFileId' must not be negative. Specified value is " + sourceFileId + ".");
		}
	...
	/**
	 * Gets a byte from the source file, taking it offset from the parameter into
	 * account plus the relative offset of the conversion routine.
	 * 
	 * @param sourceFileId The id of the source file, a non-negative integer.
	 * @param offset       The relative object of the conversion routine, a
	 *                     non-negative integer.
	 * @return The byte as integer or <code>-1</code> to indicate that the offset is
	 *         outside of the file.
	 */
		...
		try {
			...
			GraphicsPlugin.getInstance().logError(
					...
					new String[] { NumberUtility.getLongValueDecimalString(x),
							...
							NumberUtility.getLongValueDecimalString(imageData.height) },
					ex);
		}
	}
