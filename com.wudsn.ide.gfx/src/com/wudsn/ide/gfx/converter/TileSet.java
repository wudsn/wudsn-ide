...
public final class TileSet {
	 ...
	 * 
	 * @param imageData       The source image data, not <code>null</code>.
	 * @param pixelsPerColumn The number of pixels per column, a positive integer.
	 * @param pixelsPerRow    The number of pixels per row, a positive integer.
	 */
	public TileSet(ImageData imageData, int pixelsPerColumn, int pixelsPerRow) {
		...
		if (pixelsPerColumn < 1) {
			throw new IllegalArgumentException(
					"Parameter 'pixelsPerColumn' must be poitive. Specified value is " + pixelsPerColumn + ".");
		}
		if (pixelsPerRow < 1) {
			throw new IllegalArgumentException(
					"Parameter 'pixelsPerRow' must be poitive. Specified value is " + pixelsPerRow + ".");
		}
	 ...
	 * 
	 * @param column The column, a non-negative integer
	 * @param row    The row, a non-negative integer
	 * @return The tile, not <code>null</code>.
	 */
	public Tile getTile(int column, int row) {
		if (column < 0) {
			throw new IllegalArgumentException(
					"Parameter 'column' must not be negative. Specified value is " + column + ".");
		}
		if (row < 0) {
	...
	/**
	 * Create an image data instance large enough to hold the tiled source image.
	 * 
	 ...
	 * 
	 * @param targetImageData      The target image data, not <code>null</code>.
	 * @param gridColor            The pixel color for coloring the tile grid.
	 * @param inverseConflictColor The pixel color for coloring conflict tiles in
	 *                             the tile grid.
	 */
