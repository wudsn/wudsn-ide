...
public final class EnumField<T extends Enum<?>> extends Field {
	 ...
	 * @param visibleValues
	 *            The non-empty array containing the subset of enum values to be
	 *            displayed or <code>null</code> to display all values.
	 */
	public EnumField(Composite parent, String labelText,
			Class<? extends T> enumClass, T visibleValues[]) {
		if (parent == null) {
		...
		if (visibleValues != null && visibleValues.length == 0) {
			throw new IllegalArgumentException(
					"Parameter 'visibleValues' must not be empty");
		}
		...
		if (visibleValues != null) {
			for (int i = 0; i < visibleValues.length; i++) {
				...
				if (!found) {
					throw new IllegalArgumentException(
							"Parameter 'visibleValues' contain the undefined value '"
									+ visibleValue + "'.");
				}
			}
